...
public class Hold_Note : MonoBehaviour
{
    ...
    private void Update()
    {
        if (transform.parent.GetComponent<Event_F_C>().Get_F())
        {
            ...
            if (hantei_time <= Destroy_object.GetComponent<Time_time>().Return_Time())
            {
                ...
                if (Input.GetKey(key) || Input.GetKeyDown(key) || Lane_Mouse() || Lane_Mouse_Down() || Lane_Touch() || Lane_Touch_Down())
                {
                ...
                {
                    if (hantei_time + 0.1f <= Destroy_object.GetComponent<Time_time>().Return_Time())
                    {
            ...
            if (Input.GetKeyDown(key))
            {
                if (Time.timeScale == 1)
                {
                    ...
                    else if (Input.GetKey(key) || Lane_Mouse() || Lane_Touch())
                    {
                        float a = Destroy_object.GetComponent<Time_time>().Return_Time();
                        float sa = System.Math.Abs(a - hantei_time);
                        if (sa > 0.20f) { }
